...
async function initializeTelegramBot() {
    try {
        ...
        // PROFESSIONAL TEXT MESSAGE HANDLER FOR ACCOUNT CREATION
        bot.on('text', async (ctx) => {
            try {
                ...
                console.log(`Processing text from ${userId}: "${text}", step: ${ctx.session.setupStep || 'none'}`);
        ...
        if (telegramBot) {
            ...
            try {
                await telegramBot.telegram.sendMessage(
                    ...
                    `• Professional membership verification system\n` +
                    `• Enhanced group management with proper invite links\n` +
                    `• Fixed account creation flow with session management\n` +
                    `• Real-time membership checking for channels & groups\n\n` +
                    `The system is now professionally configured and ready to accept users.`,
                    { parse_mode: 'Markdown' }
                );
            } catch (error) {
