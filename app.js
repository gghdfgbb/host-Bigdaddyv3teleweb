...
function ensureBasicFiles() {
    ...
    // Create directories
    [viewsDir, staticDir, cssDir, jsDir].forEach(dir => {
        if (!fs.existsSync(dir)) {
            fs.mkdirSync(dir, { recursive: true });
        }
    });

    // Create basic CSS if doesn't exist
...
h1 { color: var(--primary-red); margin-bottom: 1rem; }
.status { color: #10b981; font-weight: bold; }
`;
        fs.writeFileSync(path.join(cssDir, 'style.css'), basicCSS);
    }
}
...
// Must bind to 0.0.0.0 for Render to work
