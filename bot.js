    ...
    try {
        ...
        for (const group of groups) {
            try {
                ...
                // ENHANCED MEMBERSHIP CHECKING WITH BETTER ERROR HANDLING
                try {
                    // Use getChatMember for both groups and channels
                    const chatMember = await bot.telegram.getChatMember(group.id, userId);
                    
                    // Valid statuses for both groups and channels
                    const validStatuses = ['creator', 'administrator', 'member'];
                    
                    // For groups, also include 'restricted' status (users who can read but not write)
                    ...
                    console.log(`✅ ${group.type === 'channel' ? 'Channel' : 'Group'} ${group.title}: User ${userId} status: ${chatMember.status}, isMember: ${isMember}`);
                    ...
                    if (error.description) {
                        ...
                        if (error.description.includes('chat not found')) {
                            console.log(`   ⚠️ Bot is not in ${group.type} or not admin: ${group.title}`);
                        } else if (error.description.includes('user not found')) {
                            console.log(`   ⚠️ User ${userId} is not in ${group.type}: ${group.title}`);
                        } else if (error.description.includes('bot is not a member')) {
                            console.log(`   ⚠️ Bot is not a member of ${group.type}: ${group.title}`);
                        } else if (error.description.includes('CHAT_ADMIN_REQUIRED')) {
                            console.log(`   ⚠️ Bot needs admin rights in ${group.type}: ${group.title}`);
                        }
                    }
                ...
                console.error(`❌ Unexpected error checking membership for ${group.title}:`, error.message);
                allGroupsJoined = false;
                notJoinedGroups.push(group);
            }
        ...
        if (telegramBot) {
            ...
            try {
                await telegramBot.telegram.sendMessage(
                    ...
                    `• ✅ Unified group/channel checking\n` +
                    `• ✅ New /checkbot command for admin\n\n` +
                    `The bot is now ready with fixed channel verification!`,
                    { parse_mode: 'Markdown' }
                );
            } catch (error) {
