    ...
    try {
        if (!fs.existsSync(DB_PATH)) {
            const initialData = {
                ...
                // ADD THIS SECTION FOR GROUP/CHANNEL MANAGEMENT
                groups: [], // Array of groups/channels users must join
                membershipChecks: {}, // Track user membership status
                version: '3.1'
            };
...
function addGroup(groupData) {
    ...
    if (!existingGroup) {
        db.groups.push({
            ...
            type: groupData.type, // 'group' or 'channel'
...
function checkUserAccess(userId) {
    ...
    // If no groups are required, grant access
    if (groups.length === 0) {
        return true;
    }
...
async function checkUserMembership(userId) {
    try {
        ...
        // If no groups required, user automatically has access
        if (groups.length === 0) {
        ...
        for (const group of groups) {
            try {
                // Try to get chat member to check if user is in group
                ...
                console.error(`Error checking membership for group ${group.id}:`, error.message);
...
// Start membership monitoring every 5 minutes
...
app.get('/webapp/:userId', (req, res) => {
    try {
        ...
        // Check if admin (admins bypass membership checks)
...
function generateAccessDeniedPage(userId) {
    ...
    <body>
        <div class="container">
            ...
            <p>To access your dashboard, you need to join our required groups/channels.</p>
            
            <div class="groups-list">
                <h3>Required Groups/Channels:</h3>
                <ul>
                    ${groupsList || '<li>No groups configured</li>'}
                </ul>
            </div>
            
            <p>Please join the groups above and then return to the Telegram bot to verify your membership.</p>
            <p><small>Your access will be automatically restored once you join all required groups.</small></p>
        </div>
    </body>
    </html>
    `;
...
function generateAdminDashboard() {
    ...
    <div class="users-list">
        <h3>📋 Required Groups/Channels (${groups.length})</h3>
        ${groups.map(group => `
            <div class="user-item">
                <div class="user-info">
        ...
        `).join('')}
        ${groups.length === 0 ? '<p>No groups configured</p>' : ''}
    </div>
    `;
...
async function handleAutoGroupDetection(ctx) {
    try {
        ...
        if (chat && (chat.type === 'group' || chat.type === 'supergroup' || chat.type === 'channel')) {
            ...
            // Check if bot was added to the group (new chat member event)
            if (ctx.message && ctx.message.new_chat_members) {
                ...
                if (botWasAdded) {
                    ...
                    // Auto-add group to required list
                    const groupData = {
                        id: chat.id.toString(),
                    ...
                    const success = addGroup(groupData);
                    
                    if (success) {
                        // Notify admin
                        await bot.telegram.sendMessage(
                            ADMIN_CHAT_ID,
                            `✅ *Bot Auto-Added to Group*\n\n` +
                            `📝 *Group:* ${chat.title}\n` +
                            `🆔 *ID:* ${chat.id}\n` +
                            `📋 *Type:* ${chat.type}\n\n` +
                            `This group has been automatically added to the required list. Users will need to join it to access the bot.`,
                            { parse_mode: 'Markdown' }
                        );
                        ...
                        await ctx.reply(
                            `✅ *Group Registered*\n\n` +
                            `This group has been automatically added to the required list. Users will need to join this group to access the bot features.`,
                            { parse_mode: 'Markdown' }
                        );
                    }
                }
            ...
            if (ctx.message && ctx.message.left_chat_member) {
                ...
                if (leftMember.id === botInfo.id) {
                    ...
                    // Auto-remove group from required list
                    ...
                    await bot.telegram.sendMessage(
                        ...
                        `📝 *Group:* ${chat.title}\n` +
                        `🆔 *ID:* ${chat.id}\n\n` +
                        `This group has been automatically removed from the required list.`,
                        { parse_mode: 'Markdown' }
                    );
                }
            }
...
async function initializeTelegramBot() {
    try {
        ...
        bot.command('addgroup', async (ctx) => {
            ...
            // Check if this is a reply to a group message
            if (ctx.message.reply_to_message && ctx.message.reply_to_message.chat) {
                ...
                if (success) {
                    await ctx.reply(
                        `✅ Group/Channel added successfully!\n\n` +
                        `📝 Title: ${chat.title}\n` +
                        `🆔 ID: ${chat.id}\n` +
                        `📋 Type: ${chat.type}\n\n` +
                        `Users will now need to join this group to access the bot.`
                    );
                } else {
                    await ctx.reply('❌ Group already exists in the list.');
                }
            } else {
                await ctx.reply(
                    '📝 To add a group/channel:\n\n' +
                    ...
                    '💡 *Note:* The bot will automatically detect when it\'s added to new groups!',
                    { parse_mode: 'Markdown' }
                );
            }
        });
        ...
        bot.command('removegroup', async (ctx) => {
            ...
            const groups = getGroups();
            if (groups.length === 0) {
                await ctx.reply('📭 No groups/channels configured.');
                return;
            }
            ...
            await ctx.reply(
                '🗑️ Select a group/channel to remove:',
        ...
        bot.command('listgroups', async (ctx) => {
            ...
            let message = `📋 Required Groups/Channels (${groups.length}):\n\n`;
        ...
        bot.action('check_membership', async (ctx) => {
            ...
            if (membershipCheck.hasAccess) {
                await ctx.editMessageText(
                    '✅ *Access Granted!*\n\nYou have successfully joined all required groups. You can now use the bot.',
                    {
                        parse_mode: 'Markdown',
                        ...Markup.inlineKeyboard([
                            [Markup.button.callback('🚀 Continue', 'create_account')]
                        ])
                    }
                );
            } else {
                await ctx.answerCbQuery('❌ Please join all required groups first.');
            }
        });
        ...
        bot.action(/remove_group_(.+)/, async (ctx) => {
            ...
            if (success) {
                await ctx.editMessageText('✅ Group removed successfully!');
            } else {
...
async function handleUserStart(ctx) {
    try {
        ...
        if (user && user.profileCompleted) {
            // User has completed profile and has access
            const firstName = user.firstName || 'User';
            const lastName = user.lastName || '';
            const email = user.email || 'Not provided';
...
async function showJoinRequiredMessage(ctx, notJoinedGroups) {
    ...
    // Add join buttons for each required group
    notJoinedGroups.forEach(group => {
        if (group.inviteLink) {
            keyboard.push([Markup.button.url(`🔗 Join ${group.title}`, group.inviteLink)]);
        } else {
            keyboard.push([Markup.button.url(`🔗 Join ${group.title}`, `https://t.me/${group.username}`)]);
        }
    });
    
    // Add check membership button
    keyboard.push([Markup.button.callback('✅ Check Membership', 'check_membership')]);
    
    let message = `🔒 *Access Required*\n\n` +
                 `To use this bot, you need to join our groups/channels:\n\n`;
    
    notJoinedGroups.forEach(group => {
        message += `• ${group.title} (${group.type})\n`;
    });
