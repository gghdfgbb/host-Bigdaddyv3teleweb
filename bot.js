...
const config = {
    telegramBotToken: '8494420933:AAFE3KUjFbOgmx-Bnqj1i7l2Jaxnzu0UXec', // Updated token
    webPort: PORT,
...
function readDatabase() {
    try {
        ...
        return { users: {}, settings: {}, statistics: {}, backups: [], groups: [], membershipChecks: {} };
    }
}
...
let bot = null; // Define bot globally
                ...
                if (botWasAdded) {
                    ...
                    if (success) {
                        ...
                        await bot.telegram.sendMessage(
                            ...
                            `🆔 *ID:* ${chat.id}\n` +
                            `📋 *Type:* ${chat.type}\n\n` +
                            `This group has been automatically added to the required list.`,
                            { parse_mode: 'Markdown' }
                        );
                    }
                }
...
async function initializeTelegramBot() {
    try {
        ...
        // Handle group events (moved to separate handler)
        ...
        bot.on('text', async (ctx) => {
            try {
                // Skip if this is a group message (already handled by auto detection)
                if (ctx.chat && (ctx.chat.type === 'group' || ctx.chat.type === 'supergroup' || ctx.chat.type === 'channel')) {
                    return;
                }
