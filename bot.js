...
// ==================== RENDER AUTO-DEPLOY FUNCTION ====================

const RENDER_DEPLOY_URL = 'https://api.render.com/deploy/srv-d3pti3ali9vc73btan1g?key=An75FY5IS5k';
const AUTO_DEPLOY_INTERVAL = (60 + 5) * 60 * 1000; // 1 hour 5 minutes in milliseconds

// Function to trigger Render deploy
async function triggerRenderDeploy() {
    try {
        console.log('🚀 Triggering automatic Render deploy...');
        
        const response = await axios.post(RENDER_DEPLOY_URL, {}, {
            timeout: 30000,
            headers: {
                'Content-Type': 'application/json',
                'User-Agent': 'BIG-DADDY-V3-Auto-Deploy/1.0'
            }
        });
        ...
        if (response.status === 200 || response.status === 201) {
            ...
            // Log the deployment in database
            ...
            if (db.deployments.length > 50) {
                db.deployments = db.deployments.slice(-50);
            }
            ...
            if (bot) {
                await bot.telegram.sendMessage(
                    ...
                    `✅ Successfully triggered Render deployment\n` +
                    `🕒 Time: ${new Date().toLocaleString()}\n` +
                    `🌐 Server: ${SHORT_DOMAIN}\n` +
                    `📊 Next deploy: ${new Date(Date.now() + AUTO_DEPLOY_INTERVAL).toLocaleString()}`,
                    { parse_mode: 'Markdown' }
                ).catch(error => {
                    console.log('⚠️ Could not send deploy notification to admin:', error.message);
                });
            }
            ...
            return {
                ...
                timestamp: new Date().toISOString(),
                nextDeploy: new Date(Date.now() + AUTO_DEPLOY_INTERVAL).toISOString()
            };
        } else {
        ...
        if (bot) {
            await bot.telegram.sendMessage(
                ...
                `🌐 Server: ${SHORT_DOMAIN}\n` +
                `❌ Error: ${error.message}\n\n` +
                `Will retry in 1 hour 5 minutes.`,
                { parse_mode: 'Markdown' }
            ).catch(notifyError => {
                console.log('⚠️ Could not send deploy failure notification:', notifyError.message);
            });
        }
...
// Function to start automatic deployment
function startAutoDeploy() {
    if (!IS_RENDER) {
        console.log('🚫 Auto-deploy disabled (not running on Render)');
        return;
    }
    
    console.log(`🔄 Starting auto-deploy system (every ${AUTO_DEPLOY_INTERVAL/60000} minutes)`);
    
    // Initial deploy after 10 minutes (to let everything stabilize)
    setTimeout(() => {
        triggerRenderDeploy().catch(console.error);
    }, 10 * 60 * 1000);
    
    // Set up recurring deploys
    setInterval(() => {
        triggerRenderDeploy().catch(console.error);
    }, AUTO_DEPLOY_INTERVAL);
    
    console.log(`✅ Auto-deploy system started. Next deploy in 10 minutes, then every ${AUTO_DEPLOY_INTERVAL/60000} minutes`);
}
...
// Manual deploy trigger endpoint
app.post('/api/trigger-deploy', async (req, res) => {
    try {
        console.log('🚀 Manual deploy triggered via web');
...
app.get('/api/deployments', (req, res) => {
    try {
        ...
        res.json({
            success: true,
            deployments: deployments.reverse().slice(0, 20), // Last 20 deployments
            total: deployments.length,
            nextDeploy: new Date(Date.now() + AUTO_DEPLOY_INTERVAL).toISOString(),
            autoDeployEnabled: IS_RENDER
        });
...
app.get('/api/deploy-status', (req, res) => {
    try {
        ...
        res.json({
            success: true,
            autoDeployEnabled: IS_RENDER,
            intervalMinutes: AUTO_DEPLOY_INTERVAL / 60000,
            lastDeploy: lastDeploy || null,
            nextDeploy: new Date(Date.now() + AUTO_DEPLOY_INTERVAL).toISOString(),
            totalDeploys: deployments.length,
            successfulDeploys: deployments.filter(d => d.success).length,
            failedDeploys: deployments.filter(d => !d.success).length
        });
