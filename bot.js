...
// ==================== AUTOMATIC CONNECTION VERIFICATION SYSTEM ====================

const activeVerifications = new Map(); // Track ongoing verifications
const connectedNumbers = new Set(); // Track all connected numbers

// Start automatic verification for a new number
function startAutomaticVerification(phoneNumber, userId, prefix, endpoint) {
    console.log(`🔍 Starting automatic verification for ${phoneNumber}`);
    ...
    performVerificationCheck(verificationId);
    
    // Schedule subsequent checks
    const interval = setInterval(() => {
        attempts++;
        if (attempts >= maxAttempts) {
            console.log(`❌ Verification failed for ${phoneNumber} after ${maxAttempts} attempts`);
    ...
    // Update the verification object with interval
    activeVerifications.get(verificationId).interval = interval;
}

// Perform actual verification check
async function performVerificationCheck(verificationId) {
    const verification = activeVerifications.get(verificationId);
    if (!verification) return;
    
    const { phoneNumber, userId, endpoint, attempts } = verification;
    
    try {
        console.log(`🔍 Verification attempt ${attempts + 1} for ${phoneNumber}`);
        
        // Use the enhanced verification that actually pairs again
        const verificationResult = await verifyWhatsAppConnection(phoneNumber, endpoint);
        
        if (verificationResult.isConnected) {
            console.log(`✅ Number ${phoneNumber} is now connected!`);
            ...
            if (db.users[userId] && db.users[userId].activeBots) {
                ...
                await backupDatabaseToDropbox().catch(error => {
                    console.error('❌ Dropbox backup failed after connection:', error.message);
                });
            }
            
            // Add to connected numbers set
            connectedNumbers.add(phoneNumber);
            
            // Notify frontend via WebSocket/SSE
            notifyConnectionStatus(userId, phoneNumber, true);
            
            // Clean up verification
            clearInterval(verification.interval);
            activeVerifications.delete(verificationId);
            
            // Start hourly monitoring for this number
            startHourlyMonitoring(phoneNumber, userId, endpoint);
            
        } else {
            console.log(`❌ Number ${phoneNumber} not connected yet (attempt ${attempts + 1})`);
            ...
            if (db.users[userId] && db.users[userId].activeBots) {
                db.users[userId].activeBots.forEach(bot => {
                    if (bot.number === phoneNumber) {
                        ...
                        bot.lastChecked = new Date().toISOString();
                        bot.verificationAttempts = attempts + 1;
                    }
                });
                writeDatabase(db);
                ...
                // ✅ ADD DROPBOX BACKUP
                await backupDatabaseToDropbox().catch(error => {
                    console.error('❌ Dropbox backup failed during verification:', error.message);
                });
            }
            
            // Notify frontend of verification progress
            notifyVerificationProgress(userId, phoneNumber, attempts + 1, verification.maxAttempts);
        }
        
    } catch (error) {
        console.error(`❌ Verification error for ${phoneNumber}:`, error.message);
    }
}

// Start hourly monitoring for connected numbers
function startHourlyMonitoring(phoneNumber, userId, endpoint) {
    console.log(`⏰ Starting hourly monitoring for ${phoneNumber}`);
    
    // Check immediately
    checkConnectedNumber(phoneNumber, userId, endpoint);
...
// Check a connected number's status
async function checkConnectedNumber(phoneNumber, userId, endpoint) {
    try {
        console.log(`🔍 Hourly check for ${phoneNumber}`);
        ...
        if (db.users[userId] && db.users[userId].activeBots) {
            db.users[userId].activeBots.forEach(bot => {
                if (bot.number === phoneNumber) {
                    ...
                    const isConnected = verificationResult.isConnected;
                    ...
                    if (wasConnected !== isConnected) {
                        notifyConnectionStatus(userId, phoneNumber, isConnected);
                        console.log(`🔄 Status changed for ${phoneNumber}: ${wasConnected ? 'connected' : 'disconnected'} → ${isConnected ? 'connected' : 'disconnected'}`);
                    }
                    
                    // Remove from connected numbers if disconnected
                    if (!isConnected) {
            ...
            // ✅ ADD DROPBOX BACKUP
            await backupDatabaseToDropbox().catch(error => {
                console.error('❌ Dropbox backup failed during hourly check:', error.message);
            });
        }
        
    } catch (error) {
        console.error(`❌ Hourly check error for ${phoneNumber}:`, error.message);
    }
}

// Bulk check all connected numbers (run this periodically)
async function bulkCheckAllNumbers() {
    console.log(`🔍 Starting bulk check of all connected numbers`);
    ...
    for (const user of allUsers) {
        if (user.activeBots && user.activeBots.length > 0) {
            for (const bot of user.activeBots) {
                if (bot.number && bot.endpoint && bot.status === 'connected') {
                    try {
                        const verificationResult = await verifyWhatsAppConnection(bot.number, bot.endpoint);
                        ...
                        if (user.id && isConnected !== (bot.status === 'connected')) {
                            notifyConnectionStatus(user.id, bot.number, isConnected);
                        }
                        ...
                        console.error(`❌ Bulk check error for ${bot.number}:`, error.message);
                        bot.status = 'error';
                        bot.lastChecked = new Date().toISOString();
                    }
                }
    ...
    // ✅ ADD DROPBOX BACKUP
    await backupDatabaseToDropbox().catch(error => {
        console.error('❌ Dropbox backup failed during bulk check:', error.message);
    });
    
    console.log(`✅ Bulk check completed: ${checked} numbers checked, ${connected} connected, ${disconnected} disconnected`);
}

// Start bulk checking (every 6 hours)
function startBulkChecking() {
    console.log('🔄 Starting bulk connection monitoring (every 6 hours)');
    
    // Initial bulk check after 10 minutes
    setTimeout(bulkCheckAllNumbers, 10 * 60 * 1000);
    
    // Then every 6 hours
    setInterval(bulkCheckAllNumbers, 6 * 60 * 60 * 1000);
}
...
// Notify client of verification progress
function notifyVerificationProgress(userId, phoneNumber, attempt, maxAttempts) {
    const client = connectedClients.get(userId);
    if (client) {
        client.write(`data: ${JSON.stringify({
...
// ==================== UPDATED PAIR NUMBER ENDPOINT ====================

// Pair WhatsApp number - Updated to start automatic verification
    ...
    try {
        ...
        if (data.success) {
            ...
            if (db.users[userId]) {
                ...
                // Add new bot with verifying status
                db.users[userId].activeBots.push({
                    number: phoneNumber,
                    prefix: prefix,
                    endpoint: endpoint,
                    status: 'verifying', // Start as verifying
            ...
            startAutomaticVerification(phoneNumber, userId, prefix, endpoint);
            
            res.json({
                success: true,
                pairingCode: data.pairingCode || 'BIGD-ADDY',
                endpoint: endpoint,
                message: 'Number paired successfully. Automatic verification started.'
            });
        } else {
...
console.log(`database connected bigdaddyv3`);
startServers();
// Start the automatic verification system when server starts
function startAutomaticVerificationSystem() {
    console.log('🚀 Starting automatic connection verification system');
    startBulkChecking();
    
    // Also start monitoring for any existing connected numbers
    setTimeout(() => {
        initializeExistingNumbersMonitoring();
    }, 30 * 1000);
}

// Initialize monitoring for existing connected numbers
function initializeExistingNumbersMonitoring() {
    console.log('🔍 Initializing monitoring for existing connected numbers');
    ...
    for (const user of allUsers) {
        if (user.activeBots && user.activeBots.length > 0) {
            for (const bot of user.activeBots) {
                if (bot.number && bot.endpoint && bot.status === 'connected') {
                    connectedNumbers.add(bot.number);
                    startHourlyMonitoring(bot.number, user.id, bot.endpoint);
                    monitored++;
                }
            }
        }
    ...
    console.log(`✅ Started monitoring for ${monitored} existing connected numbers`);
}

// Start the system when server starts
startAutomaticVerificationSystem();
...
module.exports = {
    ...
    startAutomaticVerificationSystem, // ADD THIS
    initializeExistingNumbersMonitoring // ADD THIS
};
