...
function initDatabase() {
    try {
        ...
        // If file exists, just validate and repair if needed
        console.log('🔍 Validating existing database...');
        const data = fs.readFileSync(DB_PATH, 'utf8');
        
        if (!data || data.trim() === '') {
            console.log('⚠️ Database file is empty, recreating...');
            const initialData = {
                users: {},
                settings: {
                    welcomeMessage: "👋 *Welcome to BIG DADDY V3 Bot!*\n\nTo access the bot, you need to join our sponsor channels first. After joining all required channels, you'll be able to create your account and start using the bot features.",
        ...
        try {
            ...
            writeDatabase(db);
            console.log(`✅ Database validated: ${Object.keys(db.users).length} users`);
            
        } catch (parseError) {
            console.error('❌ Database file corrupted, recreating...', parseError);
            const initialData = {
                users: {},
                settings: {
            ...
            fs.writeFileSync(DB_PATH, JSON.stringify(initialData, null, 2));
            console.log('✅ Database recreated after corruption');
        }
        ...
        console.error('❌ Critical error in initDatabase:', error);
        // Don't call readDatabase() here to avoid recursion
    }
}
        ...
        try {
            ...
            if (!data || data.trim() === '') {
                ...
                if (!newData || newData.trim() === '') {
                    throw new Error('Database still empty after reinitialization');
                }
                
                const parsedData = JSON.parse(newData);
                console.log(`✅ Database loaded after reinit: ${Object.keys(parsedData.users).length} users`);
                return parsedData;
            }
            ...
            if (!parsedData.users || typeof parsedData.users !== 'object') {
                ...
                const rebuiltData = fs.readFileSync(DB_PATH, 'utf8');
                const rebuiltParsed = JSON.parse(rebuiltData);
                console.log(`✅ Database rebuilt: ${Object.keys(rebuiltParsed.users).length} users`);
                return rebuiltParsed;
            }
            
            console.log(`✅ Database loaded: ${Object.keys(parsedData.users).length} users`);
            return parsedData;
            
        } catch (error) {
            retryCount++;
            console.error(`❌ Error reading database (attempt ${retryCount}/${maxRetries}):`, error.message);
            
            if (retryCount >= maxRetries) {
                console.error('🚨 Maximum retries reached, creating emergency database...');
                ...
                fs.writeFileSync(DB_PATH, JSON.stringify(emergencyData, null, 2));
                console.log('🚨 Emergency database created');
                return emergencyData;
            }
            
            // Wait a bit before retrying
            console.log(`🔄 Retrying database read in 1 second...`);
...
function writeDatabase(data) {
    try {
        ...
        // Limit backups array size to prevent memory issues
        if (data.backups.length > 100) {
            data.backups = data.backups.slice(-50);
        }
        ...
        console.log(`💾 Database saved: ${Object.keys(data.users).length} users`);
        return true;
        ...
        // Emergency backup
        try {
            const emergencyPath = `${DB_PATH}.emergency.${Date.now()}.json`;
            fs.writeFileSync(emergencyPath, JSON.stringify(data, null, 2));
            console.log(`🚨 Emergency backup saved: ${emergencyPath}`);
...
function performDatabaseHealthCheck() {
    ...
    Object.keys(db.users).forEach(userId => {
        ...
        if (!user || typeof user !== 'object') {
            console.log(`🗑️ Removing corrupted user: ${userId}`);
            delete db.users[userId];
            repairsNeeded = true;
        } else if (!user.id || !user.createdAt) {
            // Repair incomplete user records
            db.users[userId] = {
                id: userId,
    ...
    Object.keys(db.users).forEach(userId => {
        if (!uniqueUsers[userId]) {
            uniqueUsers[userId] = db.users[userId];
        } else {
            console.log(`🔍 Found duplicate user: ${userId}`);
            repairsNeeded = true;
        }
    });
    ...
    if (repairsNeeded) {
        console.log('🔧 Database repairs applied, saving...');
        writeDatabase(db);
    }
    
    console.log(`✅ Health check complete: ${Object.keys(db.users).length} valid users`);
    return repairsNeeded;
}

function startDatabaseMaintenance() {
    console.log('🛠️ Starting database maintenance tasks...');
    ...
    setTimeout(performDatabaseHealthCheck, 10000);
}
...
function getUser(userId) {
    ...
    if (!user.id || !user.createdAt) {
        console.log(`⚠️ User ${userId} has incomplete data, repairing...`);
        createOrUpdateUser(userId, user); // Repair user data
        return db.users[userId]; // Return repaired user
    }
...
async function startServers() {
    try {
        ...
        // Initialize database first with robust functions
        initDatabase();
        ...
        const server = app.listen(config.webPort, '0.0.0.0', () => {
            ...
            console.log(`📊 Total users in database: ${Object.keys(readDatabase().users).length}`);
        ...
        if (telegramBot) {
            ...
            try {
                await telegramBot.telegram.sendMessage(
                    ...
                    `*New Features Added:*\n` +
                    `• ✅ Robust database handling with atomic writes\n` +
                    ...
                    `The system is now fully operational with enhanced database stability!`,
                    { parse_mode: 'Markdown' }
                );
            } catch (error) {
