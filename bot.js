        ...
        bot.on('text', async (ctx) => {
            try {
                ...
                if (ctx.session.setupStep === 'asking_first_name') {
                    ...
                    // Save user profile using the stored session values
                    const firstName = ctx.session.firstName || '';
                    const lastName = ctx.session.lastName || '';
                    const email = ctx.session.email || '';
                    
                    setUserProfile(userId, firstName, lastName, email);
                    
                    // Send success message with dashboard button (use stored values)
                    await ctx.reply(
                    ...
                    // Clear session AFTER sending the message
