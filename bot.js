...
app.get('/admin/endpoints', async (req, res) => {
    try {
        // Get all endpoints and their status
        ...
        const endpointsWithStats = await Promise.all(endpoints.map(async (endpoint) => {
            ...
            try {
                const healthResponse = await fetch(`${endpoint.url}/health`, { timeout: 5000 });
            ...
            const activeSessions = await Deployment.countDocuments({ 
        ...
        res.json({ success: true, endpoints: endpointsWithStats });
    } catch (error) {
...
app.get('/admin/statistics', async (req, res) => {
    try {
        ...
        const activeDeployments = await Deployment.countDocuments({ status: 'connected' });
        const deploymentsToday = await Deployment.countDocuments({
            createdAt: { $gte: new Date().setHours(0, 0, 0, 0) }
        });
        ...
        const verificationJobs = Array.from(verificationJobs.values())
        ...
        res.json({
            ...
            statistics: {
                ...
                onlineEndpoints,
                verificationJobs,
                startupCount: global.startupCount || 0
            }
        });
    } catch (error) {
...
// Today's deployments
app.get('/admin/deployments/today', async (req, res) => {
    try {
...
// Check all endpoints
app.post('/admin/endpoints/check-all', async (req, res) => {
    try {
        const endpoints = await Endpoint.find({});
        ...
        for (const endpoint of endpoints) {
            try {
                const healthResponse = await fetch(`${endpoint.url}/health`, { timeout: 5000 });
                endpoint.status = healthResponse.ok ? 'online' : 'offline';
                await endpoint.save();
            } catch (error) {
        ...
        res.json({ success: true, message: 'All endpoints checked' });
    } catch (error) {
...
// Restart endpoint
app.post('/admin/endpoints/:id/restart', async (req, res) => {
    try {
        ...
        // Implement endpoint restart logic here
        // This would depend on your deployment setup
        
        res.json({ success: true, message: 'Endpoint restart initiated' });
    } catch (error) {
        console.error('Error restarting endpoint:', error);
        res.status(500).json({ success: false, error: error.message });
    }
});

// Stop deployment
app.post('/admin/stop-deployment/:id', async (req, res) => {
    try {
        ...
        if (telegramBot) {
            ...
            try {
                await telegramBot.telegram.sendMessage(
                    ...
                    `• 🚨 Auto-notifies when endpoints are down\n\n` +
                    `The system is now fully operational with enhanced load balancing!`,
                    { parse_mode: 'Markdown' }
                );
            } catch (error) {
