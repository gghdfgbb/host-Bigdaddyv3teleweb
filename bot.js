    ...
    try {
        ...
        bot.on('text', async (ctx) => {
            try {
                ...
                console.log(`📝 Text message from ${userId}: "${text}", chat type: ${ctx.chat?.type}, session step: ${ctx.session?.setupStep || 'none'}`);
                ...
                // Skip group messages - but ensure we're in a private chat
                if (ctx.chat && (ctx.chat.type === 'group' || ctx.chat.type === 'supergroup' || ctx.chat.type === 'channel')) {
                    console.log(`👥 Ignoring group message from ${ctx.chat.type}`);
                    return;
                }
                ...
                console.log(`🔍 Processing registration step: ${ctx.session.setupStep || 'none'}`);
                
                // Handle profile setup steps with PROPER SESSION MANAGEMENT
                if (ctx.session.setupStep === 'asking_first_name') {
                    ...
                    console.log(`✅ First name set to: ${text}, moving to last name step`);
                    ...
                    console.log(`✅ Last name set to: ${text}, moving to email step`);
                    ...
                    console.log(`✅ Email set to: ${text}, completing registration`);
                    ...
                    // If we get here, the user sent text but we're not in registration mode
                    // You can choose to ignore or handle this case
                }
