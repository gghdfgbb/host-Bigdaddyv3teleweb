...
class DatabaseManager {
  ...
  async checkUserJoinedChannels(userId) {
    ...
    // If no channels are configured, return true
...
// Improved force join check handler
async function handleForceJoin(userId, chatId) {
  // Check if admin has configured any force join channels
  if (!db.hasForceJoinChannels()) {
    return true; // No channels to join, proceed normally
  }
...
bot.onText(/\/start/, async (msg) => {
  ...
  try {
    ...
    if (userId === ADMIN_CHAT_ID) {
      const webAppUrl = `${WEB_DOMAIN}/admin`;
      const keyboard = {
        inline_keyboard: [
          [{ text: "🏠 Admin Dashboard", web_app: { url: webAppUrl } }],
