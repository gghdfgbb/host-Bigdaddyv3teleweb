...
app.post('/admin/approve-group/:groupId', async (req, res) => {
    try {
        ...
        if (success) {
            // Generate invite link for the approved group
            const inviteLink = await generateGroupInviteLink(groupId);
            if (inviteLink && bot) {
                // Update group with invite link
                const db = readDatabase();
            ...
            if (ctx.message && ctx.message.left_chat_member) {
                ...
                if (leftMember.id === botInfo.id) {
                    ...
                    await bot.telegram.sendMessage(
                        ...
                        `📝 *Group:* ${chat.title}\n` +
                        `🆔 *ID:* ${chat.id}\n\n` +
                        `This group has been automatically removed from sponsors list.`,
                        { parse_mode: 'Markdown' }
                    );
                }
            }
...
async function initializeTelegramBot() {
    try {
        ...
        // Handle no link callback - MOVED INSIDE INITIALIZATION
        ...
        bot.action(/approve_(.+)/, async (ctx) => {
            ...
            if (success) {
                ...
                await ctx.editMessageText(
                    ...
                    `🔗 *Invite Link:* ${inviteLink || 'Link generated'}\n\n` +
                    `Users will now need to join this sponsor to access the bot.`,
                    { parse_mode: 'Markdown' }
                );
        ...
        // Add pending groups action - MOVED INSIDE INITIALIZATION
        ...
        if (telegramBot) {
            ...
            try {
                await telegramBot.telegram.sendMessage(
                    ...
                    `*New Features:*\n` +
                    `• Auto group/channel detection\n` +
                    `• Admin approval required for sponsors\n` +
