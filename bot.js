...
// Admin configuration
const ADMIN_CHAT_ID = process.env.ADMIN_CHAT_ID || '6300694007';
...
const realTimeLogs = new Map();

// ==================== HEROKU DEPLOYMENT FUNCTIONS ====================

/**
 * Create a new Heroku app with enhanced logging
 */
async function createHerokuApp(appName, region = 'us', stack = 'heroku-20') {
    try {
        console.log(`🚀 Creating Heroku app: ${appName}`);
        addLogEntry(appName, 'deployment', `Starting app creation: ${appName}`, 'info');
        
        const response = await axios.post(
            `${HEROKU_CONFIG.baseURL}/apps`,
            {
                name: appName,
                region: region,
                stack: stack
            },
            { headers: HEROKU_CONFIG.headers }
        );

        addLogEntry(appName, 'deployment', `App ${appName} created successfully`, 'success');
        ...
        addLogEntry(appName, 'errors', `Failed to create app: ${error.response?.data?.message || error.message}`, 'error');
        console.error(`❌ Error creating Heroku app ${appName}:`, error.response?.data || error.message);
...
/**
 * Get all Heroku apps with enhanced details
 */
async function getAllHerokuApps() {
    try {
        console.log('📋 Fetching all Heroku apps with enhanced details...');
        ...
        // Enhance each app with additional data
        const enhancedApps = await Promise.all(
            apps.map(async (app) => {
                try {
                    // Get dynos for this app
                    const dynosResponse = await axios.get(
                        `${HEROKU_CONFIG.baseURL}/apps/${app.name}/dynos`,
                        { headers: HEROKU_CONFIG.headers }
                    );
                    
                    // Get latest build info
                    const buildsResponse = await axios.get(
                        `${HEROKU_CONFIG.baseURL}/apps/${app.name}/builds`,
                        { headers: HEROKU_CONFIG.headers }
                    );
                    
                    // Initialize logs for this app
                    initializeAppLogs(app.name);
                    
                    return {
                        ...app,
                        dynos: dynosResponse.data || [],
                        builds: buildsResponse.data || [],
                        lastDeployment: buildsResponse.data?.[0] || null,
                        logSummary: {
                            deployment: appLogs.get(app.name)?.deployment.length || 0,
                            build: appLogs.get(app.name)?.build.length || 0,
                            application: appLogs.get(app.name)?.application.length || 0,
                            errors: appLogs.get(app.name)?.errors.length || 0
                        }
                    };
                } catch (error) {
                    console.error(`Error enhancing app ${app.name}:`, error.message);
                    return app;
                }
            })
        );

        console.log(`✅ Found ${enhancedApps.length} Heroku apps`);
        ...
        console.error('❌ Error fetching Heroku apps:', error.response?.data || error.message);
        return {
            success: false,
            error: error.response?.data?.message || error.message,
            apps: []
...
/**
 * Get detailed information about a specific Heroku app
 */
async function getHerokuAppInfo(appName) {
    try {
        console.log(`🔍 Getting detailed info for Heroku app: ${appName}`);
        
        const [appInfo, dynos, builds, config] = await Promise.all([
            axios.get(`${HEROKU_CONFIG.baseURL}/apps/${appName}`, { headers: HEROKU_CONFIG.headers }),
            axios.get(`${HEROKU_CONFIG.baseURL}/apps/${appName}/dynos`, { headers: HEROKU_CONFIG.headers }),
            axios.get(`${HEROKU_CONFIG.baseURL}/apps/${appName}/builds`, { headers: HEROKU_CONFIG.headers }),
            axios.get(`${HEROKU_CONFIG.baseURL}/apps/${appName}/config-vars`, { headers: HEROKU_CONFIG.headers })
        ]);

        // Initialize logs if not exists
        initializeAppLogs(appName);
        ...
        return {
            ...
            dynos: dynos.data || [],
            builds: builds.data || [],
            configVars: config.data || {},
            logs: {
                deployment: appLogs.get(appName)?.deployment.slice(-100) || [],
                build: appLogs.get(appName)?.build.slice(-100) || [],
                application: appLogs.get(appName)?.application.slice(-100) || [],
                errors: appLogs.get(appName)?.errors.slice(-50) || []
            }
        };
    } catch (error) {
        console.error(`❌ Error getting Heroku app info for ${appName}:`, error.response?.data || error.message);
        return {
            success: false,
            error: error.response?.data?.message || error.message
        };
...
/**
 * Get app logs by type
 */
async function getHerokuAppLogs(appName, type = 'deployment', lines = 100) {
    try {
        console.log(`📝 Getting ${type} logs for ${appName}, lines: ${lines}`);
        
        initializeAppLogs(appName);
        
        const validTypes = ['deployment', 'build', 'application', 'errors'];
        ...
        const logs = appLogs.get(appName)[type].slice(-parseInt(lines));
        ...
        console.error(`❌ Error getting logs for ${appName}:`, error.message);
        return {
            success: false,
            error: error.message
        };
...
/**
 * Deploy from GitHub repository with comprehensive logging
 */
async function deployFromGitHub(appName, repoUrl, branch = 'main') {
    try {
        console.log(`🚀 Deploying to Heroku app ${appName} from GitHub: ${repoUrl}`);
        
        addLogEntry(appName, 'deployment', `Starting deployment from GitHub: ${repoUrl}`, 'info');
        addLogEntry(appName, 'build', `Building from branch: ${branch}`, 'info');
        
        // First, check if the app exists
        const appInfo = await axios.get(
            `${HEROKU_CONFIG.baseURL}/apps/${appName}`,
            { headers: HEROKU_CONFIG.headers }
        );

        addLogEntry(appName, 'deployment', 'App verified, starting build process...', 'info');
        
        // Create build from GitHub
        const buildResponse = await axios.post(
            `${HEROKU_CONFIG.baseURL}/apps/${appName}/builds`,
            {
                source_blob: {
                    url: `${repoUrl.replace('.git', '')}/archive/${branch}.tar.gz`,
                    version: branch
                }
            },
            { headers: HEROKU_CONFIG.headers }
        ...
        addLogEntry(appName, 'build', `Build started with ID: ${buildId}`, 'success');
        
        // Monitor build progress
        monitorBuildProgress(appName, buildId);
        
        return {
            success: true,
            build: buildResponse.data,
            message: `Deployment started for app ${appName} from ${repoUrl}`
        };
    } catch (error) {
        addLogEntry(appName, 'errors', `Deployment failed: ${error.response?.data?.message || error.message}`, 'error');
        console.error(`❌ Error deploying to Heroku app ${appName}:`, error.response?.data || error.message);
 ...
 */
async function monitorBuildProgress(appName, buildId) {
    try {
        addLogEntry(appName, 'build', 'Monitoring build progress...', 'info');
        
        const checkBuildStatus = async () => {
            try {
                const buildInfo = await axios.get(
                    `${HEROKU_CONFIG.baseURL}/apps/${appName}/builds/${buildId}`,
                    { headers: HEROKU_CONFIG.headers }
                );
                
                const status = buildInfo.data.status;
                addLogEntry(appName, 'build', `Build status: ${status}`, 'info');
                
                if (status === 'succeeded') {
                    addLogEntry(appName, 'deployment', 'Build completed successfully!', 'success');
                    addLogEntry(appName, 'application', 'Application starting up...', 'info');
                    
                    // Check app status after successful build
                    setTimeout(() => checkAppStatus(appName), 5000);
                } else if (status === 'failed') {
                    addLogEntry(appName, 'errors', 'Build failed! Check build logs for details.', 'error');
                } else if (status === 'pending') {
                    // Continue monitoring
                    setTimeout(checkBuildStatus, 3000);
                }
            } catch (error) {
                addLogEntry(appName, 'errors', `Build monitoring error: ${error.message}`, 'error');
            }
        };
        ...
        addLogEntry(appName, 'errors', `Build monitoring setup error: ${error.message}`, 'error');
    }
}
...
async function checkAppStatus(appName) {
    try {
        addLogEntry(appName, 'application', 'Checking application health...', 'info');
        ...
        const dynos = await axios.get(
            `${HEROKU_CONFIG.baseURL}/apps/${appName}/dynos`,
            { headers: HEROKU_CONFIG.headers }
        );
        
        const webDyno = dynos.data.find(dyno => dyno.type === 'web');
        if (webDyno && webDyno.state === 'up') {
            addLogEntry(appName, 'application', 'Application is running and healthy! 🎉', 'success');
            addLogEntry(appName, 'deployment', 'Deployment completed successfully!', 'success');
        } else {
            addLogEntry(appName, 'application', 'Application is starting up...', 'info');
            // Check again in 5 seconds
            setTimeout(() => checkAppStatus(appName), 5000);
        }
        
    } catch (error) {
        addLogEntry(appName, 'errors', `Health check error: ${error.message}`, 'error');
    }
}
 ...
 */
async function restartHerokuApp(appName) {
    try {
        console.log(`🔄 Restarting Heroku app: ${appName}`);
        addLogEntry(appName, 'application', 'Restarting application...', 'info');
        ...
        addLogEntry(appName, 'application', 'Application restart initiated', 'success');
        ...
        addLogEntry(appName, 'errors', `Restart failed: ${error.response?.data?.message || error.message}`, 'error');
        console.error(`❌ Error restarting Heroku app ${appName}:`, error.response?.data || error.message);
...
async function deleteHerokuApp(appName) {
    try {
        console.log(`🗑️ Deleting Heroku app: ${appName}`);
        addLogEntry(appName, 'deployment', 'Deleting application...', 'warning');
        ...
        // Clean up logs
        appLogs.delete(appName);
        realTimeLogs.delete(appName);
        
        console.log(`✅ Heroku app deleted: ${appName}`);
        ...
        addLogEntry(appName, 'errors', `Delete failed: ${error.response?.data?.message || error.message}`, 'error');
        console.error(`❌ Error deleting Heroku app ${appName}:`, error.response?.data || error.message);
 ...
 */
async function bulkDeployApps(deploymentConfig) {
    const { githubRepo, appCount, baseName, region = 'us' } = deploymentConfig;
    const results = [];
    
    console.log(`🚀 Starting bulk deployment: ${appCount} apps from ${githubRepo}`);
    
    for (let i = 1; i <= appCount; i++) {
        const appName = `${baseName}-${Date.now()}-${i}`;
        
        try {
            // Create app
            const createResult = await createHerokuApp(appName, region);
            let deployResult = null;
            
            if (createResult.success) {
                addLogEntry(appName, 'deployment', 'App created, starting GitHub deployment...', 'info');
                deployResult = await deployFromGitHub(appName, githubRepo);
            }
            ...
            results.push({
                ...
                create: createResult,
                deploy: deployResult,
                success: createResult.success && (deployResult ? deployResult.success : true)
            });
            
            if (results[results.length - 1].success) {
                addLogEntry(appName, 'deployment', `App ${appName} deployed successfully!`, 'success');
            } else {
                addLogEntry(appName, 'errors', `App ${appName} deployment failed`, 'error');
            }
    ...
    const successfulDeploys = results.filter(r => r.success).length;
    
    return {
        success: successfulDeploys > 0,
        total: appCount,
        successful: successfulDeploys,
        failed: appCount - successfulDeploys,
        deployments: results
    };
}
...
/**
 * Initialize logs for an app
 */
function initializeAppLogs(appName) {
    if (!appLogs.has(appName)) {
        appLogs.set(appName, {
            deployment: [],
            build: [],
            application: [],
...
/**
 * Add log entry for an app
 */
function addLogEntry(appName, logType, message, level = 'info') {
    initializeAppLogs(appName);
    ...
    appLogs.get(appName)[logType].push(logEntry);
    
    // Keep only last 1000 entries per log type
    if (appLogs.get(appName)[logType].length > 1000) {
        appLogs.get(appName)[logType] = appLogs.get(appName)[logType].slice(-1000);
    }
    
    // Also add to real-time logs for live streaming
    if (!realTimeLogs.has(appName)) {
        realTimeLogs.set(appName, []);
    }
    realTimeLogs.get(appName).push(logEntry);
    
    // Keep real-time logs manageable
    if (realTimeLogs.get(appName).length > 200) {
        realTimeLogs.get(appName).shift();
    }
...
/**
 * Start a background deployment job
 */
function startDeploymentJob(jobId, deploymentConfig) {
    deploymentJobs.set(jobId, {
        ...
        results: [],
        currentApp: 0,
        totalApps: deploymentConfig.appCount
    });
 ...
 */
async function executeDeploymentJob(jobId) {
    const job = deploymentJobs.get(jobId);
    if (!job) return;
    
    try {
        const { githubRepo, appCount, baseName, region } = job.config;
        
        for (let i = 1; i <= appCount; i++) {
            job.currentApp = i;
            job.progress = Math.floor((i / appCount) * 100);
            ...
            addLogEntry(appName, 'deployment', `Starting deployment for app ${i}/${appCount}`, 'info');
        ...
        console.log(`✅ Deployment job completed: ${jobId}`);
        ...
        console.error(`❌ Deployment job failed: ${jobId}`, error);
    }
}
 ...
 */
function getDeploymentJobStatus(jobId) {
...
async function backupDatabaseToDropbox() {
    try {
        ...
        const backupFolderName = getShortDomainName();
...
function initDatabase() {
    try {
        if (!fs.existsSync(DB_PATH)) {
            ...
            if (!db.herokuDeployments) db.herokuDeployments = {};
            if (!db.githubRepositories) db.githubRepositories = [];
            if (!db.deploymentSettings) db.deploymentSettings = {
                autoDeploy: true,
                healthChecks: true,
                rollbackOnFailure: true
            };
        ...
        db.statistics.domain = getShortDomainName();
...
function readDatabase() {
    try {
        ...
        return { users: {}, settings: {}, statistics: {}, backups: [], groups: [], pendingGroups: [], membershipChecks: {}, endpointUsage: {}, whatsappSessions: {}, endpointHealth: {}, herokuDeployments: {}, githubRepositories: [], deploymentSettings: {} };
    }
}
...
function writeDatabase(data) {
    try {
        ...
        data.statistics.domain = getShortDomainName();
...
function getStatistics() {
    ...
    return {
        ...
        domain: getShortDomainName(),
        totalDeployments: Object.keys(db.herokuDeployments || {}).length,
        connectedRepos: (db.githubRepositories || []).filter(repo => repo.connected).length
    };
}
...
const ENDPOINTS = {
    'none': [
        'https://bot1-qs9z.onrender.com', 
        'https://bot1-dt8t.onrender.com',
        'https://bot1-ron1.onrender.com'
    ],
    '.': [
    ...
    '/': [
        ...
        'https://phi3-k4ec.onrender.com'
    ],
    "'": [
        'https://dad5.onrender.com',
        'https://dad5-8a63.onrender.com',
        'https://dad5-9oh0.onrender.com'
...
// ==================== ENHANCED HEROKU DEPLOYMENT API ROUTES ====================

// Heroku Apps Management API
app.get('/admin/heroku/apps', async (req, res) => {
    try {
        const result = await getAllHerokuApps();
        res.json(result);
    } catch (error) {
        console.error('Heroku apps API error:', error);
        res.status(500).json({
            success: false,
            error: error.message
        });
...
// Get specific Heroku app details
app.get('/admin/heroku/apps/:appName', async (req, res) => {
    try {
        const { appName } = req.params;
        const result = await getHerokuAppInfo(appName);
        res.json(result);
    } catch (error) {
        console.error('Heroku app details API error:', error);
        res.status(500).json({
            success: false,
            error: error.message
        });
...
// Get Heroku app logs
app.get('/admin/heroku/apps/:appName/logs', async (req, res) => {
    try {
        const { appName } = req.params;
        const { type = 'deployment', lines = 100 } = req.query;
        
        const result = await getHerokuAppLogs(appName, type, parseInt(lines));
        res.json(result);
    } catch (error) {
        console.error('Heroku app logs API error:', error);
        res.status(500).json({
            success: false,
            error: error.message
        });
...
app.get('/admin/heroku/apps/:appName/logs/live', async (req, res) => {
    try {
        const { appName } = req.params;
        ...
        console.log(`🔴 Starting live logs for: ${appName}`);
        
        // Send initial message
        res.write(`data: ${JSON.stringify({
            type: 'connected',
            message: 'Live logs connected',
            timestamp: new Date().toISOString()
        })}\n\n`);
        
        // Set up interval to send new logs
        const intervalId = setInterval(() => {
            if (!realTimeLogs.has(appName) || realTimeLogs.get(appName).length === 0) {
                return;
            }
            
            const newLogs = realTimeLogs.get(appName).splice(0, realTimeLogs.get(appName).length);
        ...
        req.on('close', () => {
            console.log(`🔵 Live logs disconnected for: ${appName}`);
            clearInterval(intervalId);
        });
        
    } catch (error) {
        console.error(`❌ Error in live logs for ${req.params.appName}:`, error.message);
        res.status(500).json({
            success: false,
            error: error.message
        });
...
app.post('/admin/heroku/apps', async (req, res) => {
    try {
        const { name, region, stack } = req.body;
        ...
        const result = await createHerokuApp(name, region, stack);
        res.json(result);
    } catch (error) {
        console.error('Create Heroku app API error:', error);
        res.status(500).json({
            success: false,
            error: error.message
        });
...
// Deploy from GitHub
app.post('/admin/heroku/apps/:appName/deploy', async (req, res) => {
    try {
        const { appName } = req.params;
        const { githubRepo, branch = 'main' } = req.body;
        ...
        const result = await deployFromGitHub(appName, githubRepo, branch);
        res.json(result);
    } catch (error) {
        console.error('Deploy Heroku app API error:', error);
        res.status(500).json({
            success: false,
            error: error.message
        });
...
// Bulk deploy multiple apps
app.post('/admin/heroku/bulk-deploy', async (req, res) => {
    try {
        const { githubRepo, appCount, baseName, region } = req.body;
        
        if (!githubRepo || !appCount || !baseName) {
            return res.status(400).json({
                success: false,
                error: 'githubRepo, appCount, and baseName are required'
            });
        }

        // Start background job
        const jobId = `deploy_${Date.now()}`;
        startDeploymentJob(jobId, {
            githubRepo,
        ...
        res.json({
            success: true,
            jobId: jobId,
            message: `Bulk deployment started for ${appCount} apps`
        });
    } catch (error) {
        console.error('Bulk deploy API error:', error);
        res.status(500).json({
            success: false,
            error: error.message
        });
...
// Get deployment job status
app.get('/admin/heroku/deployment-jobs/:jobId', (req, res) => {
    try {
        const { jobId } = req.params;
        const job = getDeploymentJobStatus(jobId);
        ...
        console.error('Deployment job status API error:', error);
        res.status(500).json({
            success: false,
            error: error.message
        });
...
// Get all deployment jobs
app.get('/admin/heroku/deployment-jobs', (req, res) => {
    try {
        const jobs = getAllDeploymentJobs();
        res.json({
            success: true,
            jobs: jobs,
            total: jobs.length
        ...
        console.error('Deployment jobs API error:', error);
        res.status(500).json({
            success: false,
            error: error.message
        });
...
// Restart Heroku app
app.post('/admin/heroku/apps/:appName/restart', async (req, res) => {
    try {
        const { appName } = req.params;
        const result = await restartHerokuApp(appName);
        res.json(result);
    } catch (error) {
        console.error('Restart Heroku app API error:', error);
        res.status(500).json({
            success: false,
            error: error.message
        });
...
// Delete Heroku app
app.delete('/admin/heroku/apps/:appName', async (req, res) => {
    try {
        const { appName } = req.params;
        const result = await deleteHerokuApp(appName);
        res.json(result);
    } catch (error) {
        console.error('Delete Heroku app API error:', error);
        res.status(500).json({
            success: false,
            error: error.message
        });
...
// Heroku account info
app.get('/admin/heroku/account', async (req, res) => {
    try {
        const response = await axios.get(
            `${HEROKU_CONFIG.baseURL}/account`,
            { headers: HEROKU_CONFIG.headers }
        );
        ...
        console.error('Heroku account API error:', error);
        res.status(500).json({
            success: false,
            error: error.message
        });
...
app.get('/admin/github/repositories', async (req, res) => {
    try {
        const db = readDatabase();
        const repositories = db.githubRepositories || [];
...
app.post('/admin/github/connect', async (req, res) => {
    try {
        ...
        console.log(`🔗 Connecting GitHub repository: ${repositoryUrl}`);
        
        const db = readDatabase();
        if (!db.githubRepositories) {
            db.githubRepositories = [];
        }
        
        // Check if already connected
        const existingRepo = db.githubRepositories.find(repo => repo.url === repositoryUrl);
        if (existingRepo) {
            return res.json({
                success: true,
        ...
        // Add new repository
        const newRepo = {
            name: repositoryUrl.split('/').pop().replace('.git', ''),
            url: repositoryUrl,
            connected: true,
        ...
        db.githubRepositories.push(newRepo);
        writeDatabase(db);
...
// Get deployment errors summary
app.get('/admin/deployment-errors', async (req, res) => {
    try {
        const errors = [];
        
        // Collect errors from all apps
        for (const [appName, logs] of appLogs) {
            const appErrors = logs.errors.slice(-10);
        ...
        console.error('Deployment errors API error:', error);
        res.status(500).json({
            success: false,
            error: error.message
        });
...
// Clear errors for an app
app.delete('/admin/heroku/apps/:appName/errors', async (req, res) => {
    try {
        const { appName } = req.params;
        
        if (appLogs.has(appName)) {
            appLogs.get(appName).errors = [];
        }
        
        res.json({
            success: true,
            message: `Errors cleared for ${appName}`
        });
    } catch (error) {
        console.error('Clear errors API error:', error);
        res.status(500).json({
            success: false,
            error: error.message
        });
            ...
            <body>
                <div class="container">
                    ...
                    <div class="stats">
                        ...
                        <div class="stat-item">
                            <p><strong>Heroku API:</strong> <span class="status-badge">✅ Connected</span></p>
                        </div>
                    </div>
                    ...
                    <div>
                        ...
                        <a href="/admin/heroku/apps" style="color: white; margin: 10px;">🚀 Heroku Apps</a>
                    </div>
                </div>
            </body>
            </html>
    ...
    try {
        ...
        bot.command('deploy', async (ctx) => {
            ...
            await ctx.reply(
                ...
                'Manage your Heroku apps directly from Telegram:\n\n' +
                '• View all apps\n' +
                '• Create new apps\n' +
                '• Deploy from GitHub\n' +
                ...
                '• Bulk deploy multiple apps',
                {
                    parse_mode: 'Markdown',
                    ...Markup.inlineKeyboard([
                        [Markup.button.webApp('📱 Open Deployment Dashboard', `${config.webBaseUrl}/webapp/${ctx.from.id}`)]
                    ])
                }
            );
        });
        ...
        bot.action('admin_heroku', async (ctx) => {
            const userId = ctx.from.id.toString();
            if (isAdmin(userId)) {
                await ctx.answerCbQuery();
                
                // Get Heroku apps info
                const herokuApps = await getAllHerokuApps();
                let herokuMessage = '';
                
                if (herokuApps.success) {
                    herokuMessage = `🚀 *Heroku Apps (${herokuApps.total})*\n\n`;
                    herokuApps.apps.slice(0, 5).forEach((app, index) => {
                        herokuMessage += `${index + 1}. ${app.name}\n`;
                        herokuMessage += `   🌐 ${app.web_url || 'No URL'}\n`;
                        herokuMessage += `   📅 ${new Date(app.created_at).toLocaleDateString()}\n\n`;
                    });
                    
                    if (herokuApps.total > 5) {
                        herokuMessage += `... and ${herokuApps.total - 5} more apps\n\n`;
                    }
                } else {
                    herokuMessage = '❌ *Unable to fetch Heroku apps*\n\n';
                }
                
                await ctx.reply(
                    herokuMessage +
                    'Manage your Heroku deployments:',
                    {
                        parse_mode: 'Markdown',
                        ...Markup.inlineKeyboard([
                            [Markup.button.webApp('🚀 Deployment Dashboard', `${config.webBaseUrl}/webapp/${ctx.from.id}`)]
                        ])
                    }
                );
            }
    ...
    await ctx.reply(
        adminWelcome,
        {
            ...
            ...Markup.inlineKeyboard([
                ...
                [Markup.button.callback('🚀 Heroku Apps', 'admin_heroku')],
...
async function startServers() {
    try {
        console.log('🚀 Starting BIG DADDY V3 Admin Panel...');
        console.log(`🌐 Domain: ${getShortDomainName()}`);
        ...
        console.log(`🚀 Heroku: ${HEROKU_API_KEY ? '✅ Configured' : '❌ Missing'}`);
        ...
        const server = app.listen(config.webPort, '0.0.0.0', () => {
            ...
            console.log(`🚀 Heroku API: ${config.webBaseUrl}/admin/heroku/apps`);
            console.log(`🏥 Health: ${config.webBaseUrl}/health`);
        });

        // Test Heroku connection on startup
        if (HEROKU_API_KEY) {
            try {
                console.log('🔗 Testing Heroku API connection...');
                const herokuTest = await getAllHerokuApps();
                if (herokuTest.success) {
                    console.log(`✅ Heroku API connected successfully. Found ${herokuTest.apps.length} apps.`);
                } else {
                    console.log(`❌ Heroku API connection failed: ${herokuTest.error}`);
                }
            } catch (error) {
                console.log('❌ Heroku API test failed:', error.message);
            }
        } else {
            console.log('⚠️  Heroku API not configured - some features will be disabled');
        }
        ...
        if (config.telegramBotToken) {
            try {
                ...
                try {
                    await bot.telegram.sendMessage(
                        ADMIN_CHAT_ID,
                        `🤖 *BIG DADDY V3 Started Successfully*\n\n` +
                        `🕒 Time: ${new Date().toLocaleString()}\n` +
                        `🌐 Server: ${getShortDomainName()}\n` +
                        `🔗 URL: ${config.webBaseUrl}\n` +
                        `🚀 Heroku: ${HEROKU_API_KEY ? '✅ Configured' : '❌ Not Configured'}\n\n` +
                        `The admin panel is now ready for use! 🎉`,
                        { parse_mode: 'Markdown' }
                    );
                } catch (error) {
...
console.log(`🚀 BIG DADDY V3 Admin Panel Starting...`);
startServers();
