...
// ==================== FIXED REGISTRATION FORM ROUTES ====================
...
app.get('/register/:userId', (req, res) => {
    try {
        ...
        // Serve the external HTML file - NO SERVER CHECKING
...
app.post('/register/:userId', express.json(), (req, res) => {
    try {
        ...
        // Create user data for CLIENT-SIDE STORAGE ONLY
        const userData = {
            userId: userId,
            firstName: firstName,
            lastName: lastName,
        ...
        // Optional: Store minimal info on server for admin (just for tracking)
        ...
        res.json({ 
            ...
            userData: userData // Send to client for localStorage ONLY
        });
...
// ==================== FIXED WEB APP DASHBOARD ROUTES ====================

// Web App Dashboard Route (For Telegram Web App) - FIXED VERSION
app.get('/webapp/:userId', async (req, res) => {
    try {
        ...
        // 🚨 NO SERVER CHECK - Always serve dashboard
        // Let client handle localStorage checking
...
app.get('/loading/:userId', (req, res) => {
    ...
    <body>
        ...
        <script>
            // 🎯 CLIENT-SIDE LOGIC ONLY - Check localStorage and redirect appropriately
            window.addEventListener('load', function() {
                const userId = '${userId}';
                const userData = localStorage.getItem('bigdaddy_user_profile');
                
                if (userData) {
                    console.log('✅ Found localStorage data - going to dashboard');
                    setTimeout(() => {
                        window.location.href = '/webapp/' + userId;
                    }, 2000);
                } else {
                    console.log('❌ No localStorage data - going to registration');
                    setTimeout(() => {
                        window.location.href = '/register/' + userId;
                    }, 2000);
                }
            });
...
async function handleUserStart(ctx) {
    try {
        ...
        // ✅ User has access to all sponsors - ALWAYS show both options
        // Let CLIENT decide based on localStorage
        await ctx.reply(
            `🎉 *Welcome to BIG DADDY V3!*\n\n` +
            `You have access to all required sponsors.\n\n` +
            `Choose an option:`,
            {
                parse_mode: 'Markdown',
                ...Markup.inlineKeyboard([
