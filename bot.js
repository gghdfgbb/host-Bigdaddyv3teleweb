    ...
    try {
        ...
        for (const group of groups) {
            try {
                ...
                // Different checking methods for channels vs groups
                if (group.type === 'channel') {
                    // For channels, use getChatMember to check membership
                    try {
                        ...
                        // If we can't check (bot not in channel), assume user needs to join
                        isMember = false;
                    }
                } else {
...
async function initializeTelegramBot() {
    try {
        ...
        bot.command('addchannel', async (ctx) => {
            ...
            if (args.length < 2) {
                await ctx.reply(
                    ...
                    '💡 *Note:* The bot does NOT need to be in the channel for this method.',
                    { parse_mode: 'Markdown' }
                );
                return;
            }
            ...
            if (success) {
                await ctx.reply(
                    ...
                    `Users will now need to join this channel to access the bot.`,
                    { parse_mode: 'Markdown' }
                );
            } else {
        ...
        // Handle text messages for profile setup - FIXED VERSION
        bot.on('text', async (ctx) => {
            try {
                ...
                console.log(`Text from user ${userId}: "${text}", setupStep: ${ctx.session.setupStep}`);
