...
app.post('/api/pair-number', async (req, res) => {
    try {
        const { phoneNumber, prefix, endpoint } = req.body;
        
        if (!phoneNumber || !prefix || !endpoint) {
            return res.json({ 
                success: false, 
        ...
        console.log(`📱 Pairing request: ${phoneNumber} with prefix ${prefix} on ${endpoint}`);
...
app.post('/api/check-connection', async (req, res) => {
    try {
        const { phoneNumber, endpoint } = req.body;
        
        if (!phoneNumber || !endpoint) {
            return res.json({ 
                success: false, 
                error: 'Missing required parameters' 
            });
        ...
        console.log(`🔍 Checking connection for ${phoneNumber} on ${endpoint}`);
...
app.post('/api/stop-bot', async (req, res) => {
    try {
        ...
        console.log(`🛑 Stopping bot: ${phoneNumber} on ${endpoint}`);
        
        // Call delpair endpoint
        const delpairUrl = `${endpoint}/delpair?phoneNumber=${phoneNumber}`;
...
app.post('/api/restart-bot', async (req, res) => {
    try {
        ...
        console.log(`🔄 Restarting bot: ${phoneNumber} with prefix ${prefix} on ${endpoint}`);
        
        // Call pair endpoint again
        const pairUrl = `${endpoint}/pair?phoneNumber=${phoneNumber}`;
        const response = await fetch(pairUrl);
        const data = await response.json();
