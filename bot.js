...
// ==================== SIMPLE CHANNEL MEMBERSHIP CHECK ====================
    ...
    try {
        ...
        for (const group of groups) {
            try {
                ...
                // SIMPLE MEMBERSHIP CHECK - Just use getChatMember
                try {
                    ...
                    console.log(`✅ ${group.type} ${group.title}: User ${userId} status: ${chatMember.status}, isMember: ${isMember}`);
                } catch (error) {
                    console.error(`❌ Error checking ${group.type} membership for ${group.title}:`, error.message);
                    // If we can't check (bot not admin or other error), assume user needs to join
                    isMember = false;
                }
        ...
        if (chat && (chat.type === 'group' || chat.type === 'supergroup' || chat.type === 'channel')) {
            ...
            if (ctx.message && ctx.message.new_chat_members) {
                ...
                if (botWasAdded) {
                    ...
                    // For channels, use simple username format
                    let inviteLink = null;
                    if (chat.username) {
                    ...
                    if (success) {
                        ...
                        await bot.telegram.sendMessage(
                            ...
                            `🆔 *ID:* ${chat.id}\n` +
                            `📋 *Type:* ${chat.type}\n` +
                            `🔗 *Link:* ${inviteLink || 'No public link'}\n\n` +
...
async function initializeTelegramBot() {
    try {
        ...
        // Start command - USING THE WORKING VERSION FROM OLD CODE
        bot.start(async (ctx) => {
            try {
        ...
        bot.command('addchannel', async (ctx) => {
            ...
            if (args.length < 2) {
                await ctx.reply(
                    ...
                    '💡 *Note:* Use the simple @username format for channels.',
                    { parse_mode: 'Markdown' }
                );
                return;
            }
            ...
            // Create channel data with simple username link
            const channelData = {
        ...
        bot.command('addgroup', async (ctx) => {
            ...
            if (ctx.message.reply_to_message && ctx.message.reply_to_message.chat) {
                ...
                if (chat.username) {
                    ...
                    // For groups, try to generate invite link
                    try {
        ...
        // Create account process - USING WORKING VERSION FROM OLD CODE
        ...
        bot.action(/approve_(.+)/, async (ctx) => {
            ...
            if (success) {
                await ctx.editMessageText(
                    `✅ *Sponsor Approved!*\n\n` +
                    `The group/channel has been added to required sponsors.\n\n` +
                    `Users will now need to join this sponsor to access the bot.`,
                    { parse_mode: 'Markdown' }
                );
            } else {
        ...
        // Handle text messages for profile setup - USING WORKING VERSION FROM OLD CODE
...
async function showJoinRequiredMessage(ctx, notJoinedGroups) {
    ...
    // Add join buttons for each required sponsor with SIMPLE username links
        ...
        if (telegramBot) {
            ...
            try {
                await telegramBot.telegram.sendMessage(
                    ...
                    `*Fixed Features:*\n` +
                    `• Simple channel verification with username links\n` +
                    `• Working account creation flow\n` +
                    `• Enhanced membership checking\n\n` +
