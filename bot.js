...
const config = {
    ...
    maxMemoryMB: 450,
    backupInterval: 2 * 60 * 60 * 1000, // 2 hours instead of 1
...
async function backupDatabaseToDropbox() {
    try {
        ...
        if (db.backups.length > 20) { // Reduced from 50 to 20
...
// Database caching to reduce disk I/O
...
function readDatabase() {
    // Cache database for 5 seconds to reduce disk I/O
    const now = Date.now();
    if (databaseCache && (now - lastDatabaseRead) < 5000) {
...
function writeDatabase(data) {
    try {
        // Remove old backup data before writing
        if (data.backups && data.backups.length > 20) {
            data.backups = data.backups.slice(-20);
        }
        
        // Clean user data to prevent bloat
        if (data.users) {
            Object.keys(data.users).forEach(userId => {
                const user = data.users[userId];
                // Keep only essential data
                delete user.tempData;
                delete user.oldSessions;
                if (user.activeBots && user.activeBots.length > 3) {
                    user.activeBots = user.activeBots.slice(-3);
                }
            });
        }
        ...
        databaseCache = data; // Update cache
        console.log(`💾 Database written (${Object.keys(data.users || {}).length} users)`);
        return true;
    } catch (error) {
...
function createOrUpdateUser(userId, userData) {
    ...
    if (db.backups.length > 20) { // Reduced from 100 to 20
        db.backups = db.backups.slice(-20);
    }
...
function startAggressiveMemoryCleanup() {
    setInterval(() => {
        ...
        const db = readDatabase();
        if (Object.keys(db.users || {}).length % 100 === 0) {
        ...
        if (telegramBot) {
            ...
            try {
                await telegramBot.telegram.sendMessage(
                    ...
                    `• ✅ Optimized backup intervals\n` +
                    `• ✅ Improved auto-ping (5 minutes)\n` +
                    `• ✅ Emergency restart protection\n\n` +
                    `The system is now optimized for 1000+ users on Render free tier!`,
                    { parse_mode: 'Markdown' }
                );
            } catch (error) {
