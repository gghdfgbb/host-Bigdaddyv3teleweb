...
async function initializeTelegramBot() {
    try {
        ...
        bot.command('broadcast', async (ctx) => {
            ...
            const broadcastText = ctx.message.text.slice('/broadcast'.length).trim();
            if (!broadcastText) {
                await ctx.reply('Usage: /broadcast Your message here\n\nExample:\n/broadcast Hey everyone, new update is live!');
                return;
            }
            ...
            if (userIds.length === 0) {
                await ctx.reply('📭 No users in database to broadcast to.');
                return;
            }
            ...
            for (const uid of userIds) {
                // Skip sending to admin (they already see it)
                ...
                try {
                    ...
                    failed++;
                    console.log(`⚠️ Broadcast failed for ${uid}: ${e.message}`);
                }
            }
