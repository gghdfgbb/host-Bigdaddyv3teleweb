...
async function initializeDropbox() {
    try {
        ...
        // Initialize Dropbox with correct configuration
        dbx = new Dropbox({ 
        ...
        try {
            ...
            // If test fails, try to refresh token
            console.log('🔄 Attempting to refresh token...');
            const newToken = await getDropboxAccessToken();
            if (newToken) {
                // Recreate Dropbox instance with new token
                dbx = new Dropbox({ 
                ...
                try {
                    await dbx.usersGetCurrentAccount();
                    console.log('✅ Dropbox reinitialized with new token');
                    isDropboxInitialized = true;
                    return dbx;
                } catch (retryError) {
                    console.log('❌ Dropbox reinitialization failed:', retryError.message);
                    return null;
                }
            }
            return null;
...
async function backupDatabaseToDropbox() {
    try {
        ...
        // Use the makeDropboxRequest function which handles token refresh
        ...
        // If it's an auth error, try to reinitialize
        if (error.status === 401) {
            console.log('🔄 Authentication failed, attempting to reinitialize Dropbox...');
            isDropboxInitialized = false;
            dbx = null;
            ...
            try {
                await initializeDropbox();
                if (dbx) {
                    console.log('🔄 Retrying backup after reinitialization...');
                    return await backupDatabaseToDropbox();
                }
            } catch (retryError) {
