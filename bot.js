...
function approveGroup(groupId) {
    ...
    if (pendingGroup) {
        ...
        if (!inviteLink && bot) {
            try {
                // Try to create invite link (only for groups, not channels)
                if (pendingGroup.type !== 'channel') {
                    bot.telegram.createChatInviteLink(pendingGroup.id, {
                        creates_join_request: false
                    }).then(result => {
                        inviteLink = result.invite_link;
                        
                        // Update group with invite link
                        const groupIndex = db.groups.findIndex(g => g.id === pendingGroup.id);
                        if (groupIndex !== -1) {
                            db.groups[groupIndex].inviteLink = inviteLink;
                            writeDatabase(db);
                        }
                    }).catch(console.error);
                }
...
// ==================== ENHANCED MEMBERSHIP CHECK FUNCTIONS ====================
    ...
    try {
        ...
        for (const group of groups) {
            try {
                ...
                // Enhanced checking for channels vs groups
                if (group.type === 'channel') {
                    // For channels, the bot must be admin to check membership
                    try {
                        const chatMember = await bot.telegram.getChatMember(group.id, userId);
                        // For channels, 'member', 'administrator', 'creator' status means they're in the channel
                        ...
                        // If bot is not admin in channel, we can't verify membership
                        // In this case, we'll assume user needs to join and provide the link
                        if (error.description && error.description.includes('bot is not a member')) {
                            console.log(`Bot is not admin in channel ${group.title}, cannot verify membership`);
                        }
                        isMember = false;
                    }
                } else {
                    // For groups, use standard membership check
                    try {
                        const chatMember = await bot.telegram.getChatMember(group.id, userId);
...
app.post('/admin/approve-group/:groupId', async (req, res) => {
    try {
        const groupId = req.params.groupId;
        const success = approveGroup(groupId);
        
        if (success) {
            // Generate invite link for the approved group (only for groups, not channels)
            let inviteLink = null;
            const db = readDatabase();
            const group = db.groups.find(g => g.id === groupId);
            
            if (group && group.type !== 'channel') {
                inviteLink = await generateGroupInviteLink(groupId);
                if (inviteLink && bot) {
                    // Update group with invite link
            ...
            if (bot) {
                await bot.telegram.sendMessage(
                    ADMIN_CHAT_ID,
                    `✅ *Sponsor Approved*\n\n` +
                    `A new sponsor has been approved and added to the required list.\n\n` +
                    `${inviteLink ? `🔗 Invite Link: ${inviteLink}` : '🔗 Channel: Public link available via username'}`,
                    { parse_mode: 'Markdown' }
                );
            }
            
            res.json({ success: true, message: 'Group approved successfully', inviteLink: inviteLink });
        } else {
...
async function initializeTelegramBot() {
    try {
        ...
        bot.command('addchannel', async (ctx) => {
            ...
            const success = addGroup(channelData);
        ...
        bot.action(/approve_(.+)/, async (ctx) => {
            ...
            if (success) {
                // Generate invite link (only for groups, not channels)
                let inviteLink = null;
                const db = readDatabase();
                const group = db.groups.find(g => g.id === groupId);
                ...
                await ctx.editMessageText(
                    `✅ *Sponsor Approved!*\n\n` +
                    `The ${group.type} has been added to required sponsors.\n\n` +
                    `${inviteLink ? `🔗 *Invite Link:* ${inviteLink}` : '🔗 *Channel:* Public link available via username'}\n\n` +
                    `Users will now need to join this sponsor to access the bot.`,
                    { parse_mode: 'Markdown' }
                );
        ...
        // FIXED: Text message handler for account creation
        bot.on('text', async (ctx) => {
            try {
                // Skip if this is a group message
                if (ctx.chat && (ctx.chat.type === 'group' || ctx.chat.type === 'supergroup' || ctx.chat.type === 'channel')) {
                    return;
                }
                ...
                // Ensure session exists and has setupStep
                if (!ctx.session) {
                    ctx.session = {};
                }
                
                console.log(`Text from user ${userId}: "${text}", setupStep: ${ctx.session.setupStep || 'none'}`);
...
async function showJoinRequiredMessage(ctx, notJoinedGroups) {
    ...
    // Add join buttons for each required sponsor with proper invite links
    notJoinedGroups.forEach(group => {
        let link = group.inviteLink;
        
        // Generate proper link based on channel/group type
        if (!link) {
            if (group.username) {
                // For public channels with username, use t.me/username format
                link = `https://t.me/${group.username}`;
            } else if (group.type === 'channel') {
                // For private channels, we need an invite link
                // This should be provided when adding the channel
        ...
        if (link) {
            ...
            keyboard.push([Markup.button.callback(`🔗 ${group.title} (No Link)`, 'no_link')]);
        }
    });
    ...
    notJoinedGroups.forEach(group => {
        ...
        message += `• ${group.title} (${group.type})${linkInfo}\n`;
    });
        ...
        if (telegramBot) {
            ...
            try {
                await telegramBot.telegram.sendMessage(
                    ...
                    `*New Features:*\n` +
                    `• Enhanced channel support with membership checking\n` +
                    `• Manual channel addition via /addchannel\n` +
                    `• Fixed account creation flow\n` +
                    `• Improved membership verification for channels\n\n` +
