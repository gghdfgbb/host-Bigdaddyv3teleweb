...
const Endpoint = mongoose.model('Endpoint', new mongoose.Schema({
    name: String,
    url: String,
    prefix: String,
    ...
    try {
        ...
        const endpointsWithStats = await Promise.all(endpoints.map(async (endpoint) => {
            ...
            return {
                ...
                memoryUsage: endpoint.memoryUsage || 0,
                activeSessions,
                todayDeployments,
                totalDeployments
            };
...
app.get('/admin/deployments/active', async (req, res) => {
    try {
        const deployments = await Deployment.find({
            status: 'connected'
        }).sort({ createdAt: -1 }).limit(50);
...
app.get('/admin/groups/pending', async (req, res) => {
    try {
        const groups = await Group.find({ status: 'pending' }).sort({ detectedAt: -1 });
...
app.get('/admin/groups/approved', async (req, res) => {
    try {
        const groups = await Group.find({ status: 'approved' }).sort({ addedAt: -1 });
        res.json({ success: true, groups });
    } catch (error) {
        res.status(500).json({ success: false, error: error.message });
    }
...
app.get('/admin/groups/active', async (req, res) => {
    try {
        const groups = await Group.find({ status: 'active' }).sort({ addedAt: -1 });
        res.json({ success: true, groups });
    } catch (error) {
        res.status(500).json({ success: false, error: error.message });
    }
...
async function initializeTelegramBot() {
    try {
        ...
        bot.command('users', async (ctx) => {
            // Add these models after your existing User model
const Endpoint = mongoose.model('Endpoint', new mongoose.Schema({
    name: String,
    url: String,
    prefix: String,
