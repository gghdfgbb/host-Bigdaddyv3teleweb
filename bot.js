...
// Use fixed domain name for Dropbox
...
async function backupDatabaseToDropbox() {
    try {
        ...
        await makeDropboxRequest(() =>
            dbx.filesUpload({
                path: `/bigdaddyV3data/telegram_bot_database.json`,
                contents: dbBuffer,
                mode: { '.tag': 'overwrite' },
                autorename: false
            })
...
function initDatabase() {
    try {
        if (!fs.existsSync(DB_PATH)) {
            const initialData = {
                // Minimal user tracking for admin (chatId + name only)
                users: {}, // Only stores {id, firstName, lastName, isActive, createdAt}
                settings: {
            ...
            console.log('✅ Database initialized with minimal storage');
        } else {
            const db = readDatabase();
...
function createOrUpdateUser(userId, userData) {
    ...
    // Store ONLY minimal data on server
    db.users[userId] = {
        id: userId,
    ...
    if (isNewUser) {
        ...
        console.log(`✅ New user added to server: ${userId}`);
...
app.get('/register/:userId', (req, res) => {
    try {
        ...
        const serverUser = getUser(userId);
...
app.post('/register/:userId', express.json(), (req, res) => {
    try {
        ...
        // Save minimal user info to server database
        ...
        if (success) {
            ...
            res.json({ 
                ...
                userData: userData // Send to client for localStorage
            });
        } else {
            res.json({ 
                success: false, 
        ...
        <body>
                ...
                <div class="profile-card">
                    ...
                    <div class="user-info">
                        ...
                        <div class="user-details">
                            <p><strong>Name:</strong> ${serverUser.firstName} ${serverUser.lastName}</p>
                            <p><strong>User ID:</strong> ${serverUser.id}</p>
                            <p><strong>Member since:</strong> ${new Date(serverUser.createdAt).toLocaleDateString()}</p>
                            <p><strong>Storage:</strong> LocalStorage + Server</p>
                        </div>
                    </div>
                    ...
                    <form id="profileForm">
                        <div class="form-group">
                            ...
                            <input type="text" id="firstName" class="form-control" value="${serverUser.firstName}" required>
                        </div>

                        <div class="form-group">
                            <label for="lastName">Last Name</label>
                            <input type="text" id="lastName" class="form-control" value="${serverUser.lastName}" required>
                        </div>

                        <div class="form-group">
                            <label for="email">Email Address</label>
                            <input type="email" id="email" class="form-control" value="" placeholder="Email stored in your browser" required>
                        </div>
            ...
            <script>
                ...
                window.addEventListener('load', function() {
                    const userData = localStorage.getItem('bigdaddy_user_profile');
                    if (userData) {
                        try {
                    ...
                    try {
                        ...
                        if (data.success) {
                            ...
                            if (userData) {
                                ...
                                localStorage.setItem('bigdaddy_user_profile', JSON.stringify(parsedData));
                            }
...
app.post('/api/update-profile/:userId', express.json(), (req, res) => {
    try {
        ...
        // Update minimal user info on server
        const success = setUserProfile(userId, firstName, lastName, email);
...
app.get('/webapp/:userId', async (req, res) => {
    try {
        ...
        // Check if user exists in server database
        let serverUser = getUser(userId);
        
        // If user doesn't exist in server but has localStorage data, add them to server
        if (!serverUser) {
            // Check if this is a returning user with localStorage data
            // We'll handle this in the client-side JavaScript
            console.log(`🔄 User ${userId} not in server database - will check localStorage`);
        }
        ...
        if (serverUser && serverUser.isActive) {
            const membershipCheck = await checkUserMembership(userId);
            if (!membershipCheck.hasAccess) {
...
app.get('/api/user/:userId', (req, res) => {
    try {
        ...
        if (!serverUser) {
            return res.json({ 
                success: false, 
                error: 'User not found in server database',
        ...
        res.json({
            ...
            storageType: 'hybrid' // Indicates hybrid storage system
        });
...
// API endpoint to sync user from localStorage to server
app.post('/api/sync-user/:userId', express.json(), (req, res) => {
    try {
        const userId = req.params.userId;
        const userData = req.body;
        
        console.log(`🔄 Syncing user ${userId} from localStorage to server`);
        
        // Add user to server database with minimal data
        const success = createOrUpdateUser(userId, {
        ...
        if (success) {
            res.json({ 
                success: true, 
                message: 'User synced to server successfully',
                serverUser: getUser(userId)
            });
        } else {
            res.json({ 
                success: false, 
                error: 'Failed to sync user to server' 
            });
        }
...
app.post('/api/clear-verification', async (req, res) => {
    try {
        ...
        const keysToDelete = Array.from(verificationJobs.entries())
            ...
            try {
                ...
                if (response.ok) {
                    ...
                    if (data.success && data.connected) {
                        ...
                        const db = readDatabase();
                        if (db.users[job.userId]) {
                            const user = db.users[job.userId];
                            if (user.activeBots) {
                                const bot = user.activeBots.find(b => b.number === job.phoneNumber);
                                if (bot) {
...
app.get('/loading/:userId', (req, res) => {
    ...
    <body>
        <div class="loading-container">
            <div class="loading-card">
                ...
                <div class="logo-container">
                    ...
                    <span class="version">HYBRID STORAGE</span>
                </div>
                ...
                <div class="loader-wrapper">
                    ...
                    <div class="loading-subtext">Setting up hybrid storage system...</div>
                ...
                <div class="features-grid">
                    ...
                    <div class="feature-card">
                        <span class="feature-icon">🔒</span>
                        <div class="feature-text">Data Safety</div>
                    </div>
                    <div class="feature-card">
                        <span class="feature-icon">📊</span>
                        <div class="feature-text">Admin Control</div>
                    </div>
                </div>
                ...
                <div class="loading-stats">
                    <div class="stat">
                        ...
                        <span class="stat-label">Server Users</span>
                    </div>
                    <div class="stat">
        ...
        <script>
            // Check if user has localStorage data and sync with server
            window.addEventListener('load', async function() {
                ...
                if (userData) {
                    try {
                        ...
                        // Sync with server
                        const syncResponse = await fetch('/api/sync-user/' + userId, {
                            method: 'POST',
                            headers: {
                                'Content-Type': 'application/json',
                            },
            ...
            const messages = [
                "✓ Hybrid storage system initialized",
                "✓ LocalStorage connection verified", 
                "✓ Server synchronization active",
                "✓ User data safety protocols engaged",
                "✓ Admin control systems online",
...
app.delete('/admin/delete-user/:userId', (req, res) => {
    try {
        ...
        if (success) {
            res.json({ 
                success: true, 
                message: 'User deleted successfully from server database',
                note: 'User localStorage will be cleared on next access'
            });
        } else {
            res.status(404).json({ success: false, error: 'User not found in server database' });
        }
    } catch (error) {
            ...
            <body>
                <div class="container">
                    ...
                    <div class="stats">
                        ...
                        <div class="stat-item">
                            <p><strong>Storage Type:</strong> <span class="status-badge">💾 Hybrid</span></p>
                        </div>
                    </div>
...
async function handleUserStart(ctx) {
    try {
        ...
        // If user doesn't exist in server database, check if they might have localStorage data
        if (!serverUser) {
            console.log(`🔄 User ${userId} not in server database - they may have localStorage data`);
            // We'll handle this case in the web app where localStorage is available
        }
        
        const db = readDatabase();
        
        console.log(`👤 User start: ${userId}, server exists: ${!!serverUser}`);
        ...
        if (serverUser && serverUser.isActive) {
            ...
            console.log(`✅ Existing user in server database: ${userId}`);
            ...
            // User has access but no server profile - allow registration
            console.log(`🆕 User has access but no server profile: ${userId}`);
            await ctx.reply(
                welcomeMessage,
                {
                    parse_mode: 'Markdown',
...
async function listUsers(ctx) {
    ...
    if (users.length === 0) {
        await ctx.reply('📭 No users found in the server database.');
        return;
    }
    ...
    await ctx.reply(
        ...
        `💾 *Storage Note:* Users also have data in LocalStorage\n` +
        `🗑️ *Delete:* Use web dashboard to remove users`,
        {
            parse_mode: 'Markdown',
            ...Markup.inlineKeyboard([
...
async function startServers() {
    try {
        ...
        console.log(`💾 Storage: Hybrid (LocalStorage + Server)`);
        ...
        if (telegramBot) {
            ...
            try {
                await telegramBot.telegram.sendMessage(
                    ...
                    `• ✅ Users store data in LocalStorage\n` +
                    `• ✅ Server stores minimal user info\n` +
                    `• ✅ Admin can view/manage all users\n` +
                    `• ✅ No data loss on server restart\n` +
                    `• ✅ Automatic user sync from LocalStorage\n\n` +
                    `*Smart Features:*\n` +
                    `• 🔄 Health checks every 5 minutes\n` +
                    `• ⚡ Load balancing across endpoints\n` +
                    `• 📊 WhatsApp session monitoring\n` +
                    `• 🚨 Auto-notifications for issues\n\n` +
                    `The system is now fully operational with hybrid storage!`,
                    { parse_mode: 'Markdown' }
                );
            } catch (error) {
