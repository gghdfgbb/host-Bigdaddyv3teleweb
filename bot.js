...
// Fixed API endpoint for checking connection
app.post('/api/check-connection', async (req, res) => {
    ...
    try {
        ...
        try {
            // FIRST: Check our database sessions (fastest)
            const sessionKey = `${userEndpoint}_${phoneNumber}`;
            const sessionData = db.whatsappSessions?.[sessionKey];
            
            if (sessionData) {
                console.log(`📊 Found session data in database for ${phoneNumber}:`, sessionData.isConnected);
            ...
            // SECOND: If no session data, check the endpoint directly
            console.log(`🔍 No session data in database, checking endpoint directly: ${userEndpoint}`);
            
            const sessionsUrl = `${userEndpoint}/sessions`;
            console.log(`🌐 Calling endpoint: ${sessionsUrl}`);
            
            const response = await fetch(sessionsUrl, { 
                timeout: 15000,
                headers: {
                    'User-Agent': 'BIGDADDY-V3-Bot'
                }
            });
            ...
            const data = await response.json();
            console.log(`📡 Endpoint response for ${phoneNumber}:`, data);
            
            if (data.success && data.sessions && Array.isArray(data.sessions)) {
                // Look for the specific phone number in sessions
                const userSession = data.sessions.find(session => 
                    session.phoneNumber === phoneNumber
                );
                
                const isConnected = userSession ? (userSession.isConnected === true) : false;
                
                console.log(`📊 Sessions found: ${data.sessions.length}, User ${phoneNumber} connected: ${isConnected}`);
                
                // Update session data in our database
                if (!db.whatsappSessions) db.whatsappSessions = {};
                ...
                if (userSession) {
                    db.whatsappSessions[sessionKey] = {
                        ...
                        endpoint: userEndpoint,
                        mode: userSession.mode || 'unknown',
                        health: userSession.health || 'unknown',
                        messagesProcessed: userSession.messagesProcessed || 0,
                        errors: userSession.errors || 0,
                        queueSize: userSession.queueSize || 0,
                        welcomeSent: userSession.welcomeSent || false,
                        lastActivity: userSession.lastActivity || 'unknown',
                        isConnected: isConnected,
                        lastUpdated: new Date().toISOString()
                    };
                } else {
                    // Create basic session entry even if not found
                    db.whatsappSessions[sessionKey] = {
                        phoneNumber: phoneNumber,
                        endpoint: userEndpoint,
                        mode: 'unknown',
                ...
                console.log(`❌ Invalid response format from endpoint:`, data);
                return res.json({
                    success: false,
                    error: 'Invalid response format from endpoint',
                    isConnected: false
                });
            }
